...
if st.button(f"Show one random triplet"):
    # Randomize an elem and update session state
    st.session_state.dataID = random.choice(list(dataset.keys()))


# Determine which ID to display: prioritize manual input if available
displayID = st.session_state.dataID
...
if displayID in dataset:
    ...
    st.write("This annotation ID does not exist or the search has no results.")
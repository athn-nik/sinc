...
from space.data.tools.collate import collate_length_and_text
import space.launch.prepare
from space.render.mesh_viz import visualize_meshes
from space.render.video import save_video_samples, stack_vids
import torch
from space.transforms.base import Datastruct
from space.utils.inference import cfg_mean_nsamples_resolution, get_path
from space.utils.file_io import read_json, write_json
...
def calc_temos_score(newcfg: DictConfig) -> None:
    ...
    if cfg.set == 'submission':
        from space.utils.inference import sinc_eval_set
        keyids = sinc_eval_set
    elif cfg.set == 'small':
        from space.utils.inference import validation_nostand_notrain
        keyids = validation_nostand_notrain
    elif cfg.set == 'supmat':
        from space.utils.inference import sinc_supmat
        keyids = sinc_supmat
    else:
        if cfg.set == 'pairs':
    ...
    with torch.no_grad():
        with tqdm(total=len(keyids), position=0, leave=True) as pbar:
            ...
            for keyid in (pbar := tqdm(keyids,  position=0, leave=True)):
                ...
                from space.data.tools import collate_text_and_length

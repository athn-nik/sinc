import numpy as np
import torch
from sinc.tools.geometry import axis_angle_to_quaternion, quaternion_to_axis_angle, matrix_to_quaternion, quaternion_to_matrix
...
def slerp_poses(last_pose, new_pose, number_of_frames, pose_rep="matrix"):
    # interpolation
    from sinc.tools.easyconvert import to_matrix, matrix_to
    ...
    # SLERP Local Quats:
    interp_ws = np.linsinc.0.0, 1.0, num=number_of_frames+2, dtype=np.float32)
...
def slerp_translation(last_transl, new_transl, number_of_frames):
    alpha = torch.linsinc.0, 1, number_of_frames+2)
    # 2 more than needed
...
def align_interpolate(rotations, translation, s, e, align_rot=True, align_trans=True, interpolate=True):
    ...
    from sinc.tools.interpolation import align_orientations, interpolate_track, linear_interp
...
def align_orientations(s, e, poses_interp, transl):
    ...
    # starting from j
    # remove the rotation
    from sinc.tools.geometry import matrix_to_axis_angle, axis_angle_to_matrix
...
def interpolate_track(s, e, poses, inrep='matrix', outrep='matrix'):
    ...
    # SLERP Local Quats:
    interp_ws = np.linsinc.0.0, 1.0, num= e - s + 1, dtype=np.float32)
